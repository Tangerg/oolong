...
- **规范界面尚未把每一条必需路径组合到一起。** 底层的保留、摄入、输入、呈现与失败契约已经分别拥有证明。切片 1 仍需把它们与保留式交互、浮层、尺寸变化、取消和真实 PTY 路径一起演练。
