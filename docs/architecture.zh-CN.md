...
已经实现的边界刻意保持很小。活动的 `headless.Widget` 绘制到 `headless.Frame`；后者嵌入已裁剪的 `grid.View`，并通过 `Sub` 和 `Subs` 把根事务传给子组件。`headless.Root` 是连接运行时消费方定义的 `program.Component` 的唯一适配器；它在整棵树返回后提交所有嵌套 `Snapshot`，并在绘制 panic 时放弃待定状态。`Scroll.Stage` 与 `Transcript.Stage` 把同一事务应用到派生的视口边界和 transcript 重排，因此这些更低层、可复用的模型也不会提前发布新的坐标空间。

被动的已完成内容是另一份契约：`headless.Block` 直接绘制到 `grid.View`、自行测量，并且不拥有路由几何。当一个 block 必须出现在活动组件树中时，`headless.Static` 是显式桥梁。需要几何查询的被动 block 把相关约束作为参数接收；它不会在绘制时写入不可见的命中表。
...
复制/搜索边界证明了这个依赖方向。一个视觉文本行现在是 `core/text.Row`：有意义的文本、它在渲染结果中的起始列，以及可以逆转换行的元数据。`markdown` 与 `components` 都能产生它，而不需要任何一方学习另一方的词汇。Transcript 的选择与搜索消费这个更底层的值；gutter、marker 和组件身份不会泄漏进复制文本。
...
窗口尺寸是会话关键输入，不是可选探测。对每个受支持的平台，打开之后发生的尺寸变化最终都**必须**和打开时的 `Resize` 一样进入同一条有序输入流。原生信号是首选；平台没有独立信号时，终端适配器**必须**在不为输入句柄制造第二个拥有者的前提下轮询或合成变化。尺寸属于可替换状态，所以观察结果**可以**合并，但最新观察到的尺寸不得丢失。既没有可靠信号、也没有有界且可停止的观察器的平台，不属于受支持的终端宿主。
...
| 测量与绘制在可观察意义上纯 | [`headless`](../components/headless/draw_purity_internals_test.go) 与 [`kit`](../components/kit/draw_purity_internals_test.go) 对每个生产 `Draw` 接收者分类；每个有状态接收者从同一份有意义状态绘制两次，语义投影保持不变，并产生相同的终端字节、样式和光标状态 | 每次组件测试；未分类的接收者会失败 |
| 第 6.3 节的单帧路由几何 | 一条路由测试在新根绘制暂存时观察到旧快照，根提交之后观察到完整的新快照；任何子节点几何的混合都不可见 | 切片 2 的必需关卡 |
| 受支持平台的尺寸变化投递 | 真实 Unix PTY 改变几何后必须产生后续 `Resize`；Windows 轮询状态机使用确定性时钟测试变化检测、错误恢复、去重与停止；Windows 源码在 CI 中构建并测试 | 每次终端测试以及每个受支持的操作系统源码集合 |
| 空闲时绘制与发布工作为零 | [`TestAnIdleProgramStopsWriting`](../core/program/program_test.go) 与定时器测试证明不存在无条件帧时钟和重复字节；必须采样外部状态的平台观察器保持有界，观察结果不变时不发事件，并随会话停止 | 每次 CI |
| 失败与所有权结算 | [`program` 故障测试](../core/program/program_test.go)覆盖输入 cause、部分输出、不再后续写入、排空超时和能力缺席；[`term` 故障测试](../core/term/terminal_test.go)覆盖真实 PTY 收尾，[`Writer`](../core/term/writer_test.go) 覆盖短写/部分写和有界关闭 | 切片 1 以及每个新宿主的必需关卡 |
| 公开模块兼容性 | 每个模块脱离 `go.work` 构建；发布工作流使用固定版本的 `golang.org/x/exp/cmd/gorelease` 与前一个不可变模块 tag 比较，在 pre-1.0 报告变化，并拒绝违反 Go 兼容性的 v1+ 候选 tag；常规 CI 检查声明的 Go 地板和受支持的源码集合 | tag 前手动运行发布检查，并对每个公开模块 tag 运行 |
...
- 空闲界面不做绘制或发布工作，不写字节，也不跑无条件的帧时钟；无法用信号报告必要外部状态的平台**可以**使用一个有界观察器，但它只发送变化，并随会话停止；
- 每个定时器至多有一次待处理的 tick；
- 输入没变时不重复测量或折行。
...
4. 每个公开模块的每一个导出标识符，都完成了所有权、零值、错误、并发和依赖方向评审。不可变模块 tag 建立公开 API 基线，固定版本的 `gorelease` 发布检查会把每个候选版本与前一份基线比较。
...
现有基础的大部分已经指向这个方向：inline 发布、有界的 transcript 释放、带因果结果的输入流、有界的字节摄入、故障注入的所有权结算、组件侧原子呈现快照、事务化的滚动与 transcript 重排、活动 widget 与被动 block 的区分、单拥有者运行时、复合式的对话框与 tabs 控制器、结构化组件语义、增量 markdown、裁剪过的 cell view、跨平台尺寸观察、消费方定义的能力、可执行的绘制纯度分类、发布 API 检查，以及被强制执行的依赖 DAG，都是要保住的资产。

### 14.1 已知的不变式违规

下列问题与本文的一条“必须”相抵触，并且在 v1 前**必须**清空：

- 当前没有已知条目。新发现的违规必须立即写在这里；这张空清单是一项可测试的发布条件，并不声称未来审计不可能再发现问题。

### 14.2 缺失的能力与证明

下列问题不是当前的不变式违规，而是系统仍然缺少的能力或端到端证明：

- 当前没有已知条目。可选切片仍然取决于条件；在它们所写明的证据证明有必要之前，它们不属于缺失的 v1 基础。
...
| --- | --- | --- | --- |
| 1 | 已交付载荷保留、输入失败、有界流摄入、端到端失败语义 | 规范流、保留关卡、故障宿主、PTY | 已完成（必需） |
| 2 | 原子提交几何与呈现状态所有权 | 跨嵌套组件的旧/待定/新路由快照测试 | 已完成（必需） |
| 3 | 复合 headless 所有权、共享语义与精致的 `kit` 组合 | 同一个模式由两个控件证明 | 已完成（必需） |
...
`Host.Input` 会返回一条带因果结果的 `EventSource`：通道关闭之后可以读取 `Err` 结果，因此干净 EOF 与已知传输失败保持可区分，同时 `program` 不耦合到任何具体宿主。`ByteIngress` 提供无损字节政策：它的子进程调用方证明了批量化、待处理字节上限、生产者背压、完成与失败的顺序，以及不改变通用 dispatcher 的 owner 取消。`Transcript.Commit` 会真正释放已交付载荷与逐块放置记录，同时保留一个聚合的活动坐标基准；它的确定性保留测试以及独立进程 `N` 对 `2N` 的 GC 测试，是这个切片**必须**保持为绿的关卡。故障注入覆盖来源与输入失败、有歧义的部分输出写、不再后续写入、排空超时、能力缺席、有界关闭，以及真实终端上的尽力收尾。

这个切片已经完成。[`examples/streaming`](../examples/streaming) 是唯一一条规范路径，而不是并排展示功能的样品：批准在来源启动前转移焦点；来源只能通过 `ByteIngress` 写入；`markdown.Stream` 把变化中的后缀转成稳定块；`Transcript.CommitN` 只交付超出上限的完成前缀，同时保留固定大小的近期窗口，用于选择、指针与滚动交互。取消是一种与来源失败不同的领域结果，而收尾则通过 ingress 所有权取消来源。确定性的示例测试证明批准顺序、稳定/开放转换、保留上限、失败前已接受的字节以及取消；组件测试证明滚轮与选择路由；真实 PTY 套件证明尺寸变化、空闲静默、模式恢复、scrollback 存续，以及从批准到发布的完整路径。program 与 terminal 的故障套件继续作为输入关闭、有歧义的部分写、排空超时、可选能力缺席和相互独立的收尾尝试的可执行证据。
...
这个切片已经完成。`Root`、`Frame` 与 `Snapshot` 提供组件拥有的提交边界；容器、栈、字段、列表、编辑器、视口和带外观的控件都通过它进行路由。`Scroll.Stage` 与 `Transcript.Stage` 让视口边界和依赖宽度的 transcript 放置处于同一个事务中。必需的嵌套测试同时改变外层和内层布局，在绘制过程中把事件路由到旧组合，在提交后观察完整的新组合，并证明两种新旧混合组合都不可见。panic 测试则证明待定值及其引用会被释放，而上一帧仍保持为当前帧。
...
这个切片已经完成。`headless.Dialog` 拥有一个模态状态机；content 与 trigger 是它的复合部件，每一条关闭路径都会结算受控或本地的 open 状态，而 `Stack.Remove` 可以关闭被覆盖的对话框而不误关更新的浮层。焦点测试证明，打开时基础界面会失去键盘所有权，每一种关闭方式都会恢复它。`headless.Tabs` 拥有自己的 tab 部件以及受控或本地的选择状态；调用方直接写入受控状态后通过显式 `Sync` 应用，因此绘制不会偷偷完成焦点转移。两个控件都投影为同一棵带类型的 `SemanticNode` 树，而不暴露视觉方框。`kit.NewDialog` 与 `kit.NewTabs` 是精致、可主题化的短路径，同时控制器与外观部件仍然可以直接访问。结构化测试覆盖了角色、selected/open/focused 状态以及两种所有权模式。
