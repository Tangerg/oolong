...
[grok-build 的 `xai-ratatui-inline`](prior-art.md#4-grok-build-a-counter-example-which-is-more-useful-than-a-borrowing) 正在交付的恰好就是这种清除后重绘的替代方案。点名一个可工作的反例不会削弱这条规则；它记录的是 Oolong 明确拒绝了哪一项真实权衡。
