...
**在一个完整的子进程或网络驱动的示例证明了所需策略之前，不新增任何通用的 mailbox、observable 或 stream 框架。** 真要加的时候，它应当建立在现有的 dispatcher 和任务队列之上，而不是造出第二个拥有者循环。
...
现有基础的大部分已经指向这个方向：inline 发布、单拥有者运行时、增量 markdown、裁剪过的 cell view、消费方定义的能力、以及被强制执行的依赖 DAG，都是要保住的资产。

### 14.1 已知的不变式违规

下列问题与本文的一条“必须”相抵触，并且在 v1 前**必须**清空：

- **已交付的 transcript 载荷仍被保留。** `Transcript.Commit` 推进一个已交付索引并停止绘制那些块，但切片仍然持有块引用和全部放置记录。终端已经取得了呈现所有权，而应用内存仍会随会话年龄增长。持久的修法必须释放已交付的载荷，并在不让活坐标产生歧义的前提下压缩或概括过时的放置状态。
- **几何在绘制过程中变得可观察。** 容器、栈、字段和带外观的组件在绘制时记录命中测试几何。数据本身是正确派生的，但还没有一个明确的提交点，把路由快照绑定到被接受用于呈现的那一帧上。持久的修法应当确立那条边界，而不是在现有缓存旁边再加一个缓存。
- **输入传输失败没有保留 cause 的路径。** `Host.Events` 关闭后，`Run` 当前把每次关闭都当成干净的会话结束。明确知道 SSH、管道或终端读取失败的宿主无法保留该 cause。切片 1 **必须**让干净 EOF 与已知传输失败可区分，同时不让 `program` 耦合到某个具体宿主。

### 14.2 缺失的能力与证明

下列问题不是当前的不变式违规，而是系统仍然缺少的能力或端到端证明：

- **通用分发没有流背压政策。** `Dispatcher.Post` 正确地避免阻塞拥有者，但快速的生产者可以造出一个无界的闭包 FIFO。流式摄入仍然需要第 8 节描述的语义分类和端到端证明。修法属于更高层的流摄入对象；非阻塞 dispatcher 契约保持不变。
...
这个切片同时负责 transcript 与输入失败两项违规，以及缺失的流摄入政策。`Transcript.Commit` **必须**真正释放已交付载荷与逐块放置记录，同时保留一个聚合的活动坐标基准。流摄入**必须**批量化无损 chunk，限制生产者领先量，保持完成与错误的顺序，并在取消时不泄漏 goroutine。第 10 节的确定性保留测试以及独立进程 `N` 对 `2N` 的 GC 测试在这里成为 CI 关卡。一个只是画出了正确最终屏幕、却仍然保留整个会话的演示不算完成。
