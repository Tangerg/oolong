...
13. **泛型消除重复的算法。** 它们不用来造组件类型层次或通用状态容器。当存储状态依赖某个类型时，把类型参数放在接收者类型上；当一个具体接收者拥有某项操作、但只有本次调用才引入类型时，把类型参数放在方法上。没有真实接收者的构造与适配仍然是包级函数；绝不为了使用方法语法而虚构工厂、工具或命名空间接收者。
14. **每个层级的一项语义操作只有一个入口。** 当可用零值或显式参数已经能让规范调用清晰时，不要再导出默认包装、`With` 变体、丢失信息的适配器或便利别名。第二个入口必须跨越一个说得清的边界——所有权、生命周期、抽象层级或结果契约——而不能只为了替调用方少写一个参数。契约变化时，更新调用方并删掉过时路线。
