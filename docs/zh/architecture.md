...
被保留的函数值必须有明确阶段。**投影回调**可以在测量或绘制期间运行，并继承同一份可观察纯度契约。**语义回调**在拥有者重建有意义状态时运行，**事件回调**从输入处理中运行。库只保留由自己调用的函数；不透明的调用方值可以跨越泛型边界携带应用行为，而不让该函数成为渲染模型的一部分。库可以强制自己在哪个阶段调用回调，也可以拒绝其完整包内渲染调用图里的固有副作用；它无法检查下游投影回调的函数体。因此，公开回调文档与调用方的组件测试必须承担契约的最后一部分。
...
时间边界必须说明自己回答的是两种问题中的哪一种。**截止时间**是所有权边界：到达截止时刻时，旧操作已经不再存活，因此区间是左闭右开的（`now < deadline`）。**交互窗口**根据事件到达时间进行分类：不晚于给定上限的事件仍可以属于同一个手势或按键序列，因此上边界可以包含在内。公开文档和边界测试必须写明这一选择。当多个操作判断同一份截止所有权时，它们必须共享一个 predicate，而不能分别重复一对可能在精确时刻产生分歧的反向比较。
...
访问器始终表示调用方的当前值，绝不能只是一个初始化种子。绘制可以读取并投影它，但不得同步派生的语义状态。字段在下一次语义操作时收敛自己的编辑器、光标或选择集合。若控制器的收敛还必须转移焦点或改变模态栈成员关系，它就暴露 `Sync`，因为访问器被刻意设计成不是事件订阅；`Sync` 会报告相关状态是否改变。这是一套带显式转移边界的所有权模型，而不是第二套状态 API。

### 6.3 布局与已提交的几何

`core/layout` 保持为纯几何。它划分尺寸并返回矩形；它不认识 widget、焦点、主题或事件路由。

`grid.View` 拥有的是被裁剪的绘制意图，不是一段彼此隔离的 cell 存储。一个操作必须先与 clip 相交才可以生效，但多列显示原子在存储和外观上都是不可分割的事实。通过其中任意一列替换内容时，必须把整个旧原子清空，包括 clip 外的部分，同时保留外侧各列原有的样式。外观属于该原子的 head，也就是 painter 唯一会发出的列：外观区域包含 head 时，必须改变整个原子的样式或链接，包括 clip 外的 continuation；只包含 continuation 时则什么也不做。因此，即使面对非幂等的混色，相邻区域也只会把外观划分并应用一次。这两种情况是唯一允许发生在 view 的 clip 外的变更，grid 的白盒属性测试会强制这两项区别与分区定律。
...
| 不变式 | 可执行的证据 | 关卡 |
| --- | --- | --- |
| 依赖方向与词汇 | `internal/arch` 从声明的 DAG 推导出被禁止的 import，并检查模块承诺、文档中限定与未限定的具体类型引用、完整性与无环性 | 每次 CI |
...
| 明确的时间边界语义 | clipboard 测试证明请求所有权采用左闭右开区间，并在精确截止时刻过期；keymap 测试证明交互序列包含恰好在超时时刻到达的 chord。两者都直接测试文档声明的边界，而不依赖调度时机 | 每一个新的截止时间或交互窗口 |
| 可观察意义上纯粹的测量与绘制 | [`headless`](https://github.com/Tangerg/oolong/blob/main/components/headless/draw_purity_internals_test.go)、[`kit`](https://github.com/Tangerg/oolong/blob/main/components/kit/draw_purity_internals_test.go)、[`markdown`](https://github.com/Tangerg/oolong/blob/main/markdown/draw_purity_internals_test.go) 和 [`latex`](https://github.com/Tangerg/oolong/blob/main/latex/draw_purity_internals_test.go) 从源码推导每一个生产用的 `HeightForWidth` 与 `Draw*` 接收者，并逐一执行两次；有状态用例保持语义投影不变，全部用例都返回相同尺寸和帧，事件回调计数也属于被观察状态。[`internal/arch` 渲染副作用关卡](https://github.com/Tangerg/oolong/blob/main/internal/arch/render_effects_internals_test.go)从这些入口沿类型解析后的包内调用图追踪，拒绝 goroutine、I/O、时钟、随机性、日志、发布和非投影回调。[回调阶段关卡](https://github.com/Tangerg/oolong/blob/main/internal/arch/render_callbacks_internals_test.go)要求每一个被保留的函数值都必须有且只有一条不过期的投影、语义或事件声明 | 每一个实现测量或绘制的包；未分类接收者、回调、固有副作用、未解析本地调用或阶段违规都会直接失败 |
...
| 语义状态与焦点只有一个拥有者 | headless 中由源码推导的受控状态测试要求该包每一个导出的 `Accessor` owner 证明调用方写入转移、实际接受的写入以及 no-op 写入。kit 中由源码推导的组合测试要求每一个接受这条所有权接缝的 Config 都提供一个可执行的双向 controlled example；[`internal/arch`](https://github.com/Tangerg/oolong/blob/main/internal/arch/facade_internals_test.go) 另行拒绝 kit 方法镜像 controller 或 editor 的部分状态 API。组件所有权测试要求重复安装可证明为同一身份的孩子及重复报告同一焦点状态保持幂等，而模块私有的身份原语为开放组件接口提供共享的保守比较 | 每次 CI，以及每一个新的受控 owner 或组件包装器 |
| 唯一的显示原子几何 | 宽度 fixture 是确定性估算器的唯一 oracle，钉住它对歧义宽度、emoji、组合符、半宽片假名、已知 spacing mark 边界以及超过两列情形的答案。第一道白盒 fuzz target 通过完整和被裁剪的 view 施加全部六种公开 cell 变更操作；它证明每个 head 精确拥有自己的 continuation 列、每一列共享该原子的样式与链接、内容替换只能在 clip 外以保留样式的方式清除相交的旧原子，外观变更也只能改变 head 归该 clip 所有的完整原子。第二道差分 target 证明：对相邻分区分别施加 Blend 或 Fade，等于对它们的并集只施加一次。`ptytest.Screen` 刻意共享同一估算器，并独立建模终端侧的应用、延迟换行、覆盖、擦除与滚动 | 每次 text/grid 测试，以及每次宽度表更新 |
| 可变所有者始终只有一个拥有者 | `internal/arch` 从生产文档中推导所有声明不可复制的导出类型，并要求它直接带有 `noCopy`、`sync` 或 `sync/atomic` 字段。第二道由源码推导的检查会跨仓库包沿按值结构体包含关系传播，并拒绝导出的包装类型在没有声明并标记自身所有权的情况下继承 copylocked 身份；随后 `go vet` 会在共享可变存储取得两个拥有者之前拒绝值复制 | 每次 CI，以及每一个新的可变所有者或不可复制契约 |
...
| 公开模块兼容性 | 每个模块在没有 `go.work` 的情况下构建；`scripts/modules.sh` 从 `go.work` 推导完整成员关系，并从受支持源码集里可被外部导入的生产包推导公开成员关系。清单是原子且失败关闭的：全部源码集都必须可读取，空的公开集合非法，诊断不会进入数据流，消费方取得成功结果后才能解释它。`internal/arch` 会执行这些失败路径与符号链接检出路径，独立读取 Go 的包事实，并要求两个集合都精确相等。经过测试的 `internal/cmd/apiledger` Go 命令消费这份公开集合，对没有已发布基线的模块作出报告而不是静默省略，并使用钉住版本的 `apidiff`，在 Linux、macOS 与 Windows 上把每个已发布模块同前一个不可变 tag 比较，要求每个不兼容的导出 API 变更以精确名称出现在所选迁移清单中。发布流程还会运行钉住版本的 `gorelease`，报告 pre-1.0 变更并拒绝违反 Go 兼容性的 v1+ tag 提案 | 每次 CI、打 tag 前的手动发布检查，以及每一个公开模块 tag |
| 源码可移植性与散文完整性 | 每个模块都会面向 `wasip1/wasm` 与 `js/wasm` 执行 vet 和 build，证明所选源码与依赖在这些目标上仍能通过类型检查，但不声称运行时无 I/O 或已有 WebAssembly 终端适配器；钉住版本的 CSpell 检查人工编写的 Markdown 与 NOTICE 词汇，而不会用生成标识符稀释关卡 | 每次 CI 与发布 |
...
8. **配置是显式的值。** 构造过程有一组相关的声明式设置时，接受包级 `Config` 结构体，并让其中的可选字段具有实用的零值语义。不要引入函数式选项 API。
...
12. **panic 只针对调用方自己的错误，绝不针对别人的数据。** 一次 panic 说明程序写错了——构造函数无法替代的 nil 依赖、两条互相矛盾的边界、在一帧内部又画了一帧、用尽的身份计数器。终端发来的、用户键入的、文件里存的、网络返回的，都不允许触达任何一处；解析器的 fuzz 目标就是为了让这一点持续成立。每一条会 panic 的前置条件都写在带有它的那个导出 API 的文档注释里，连同「继续执行会付出什么代价」一并说明——一个在 `go doc` 里看不到这条规则的调用方，只能在运行时学到它，而那正是这次 panic 本想避免的结果。
13. **注释写为什么，不写是什么。** 旁边的代码已经说了发生了什么。文档注释是唯一的例外：陈述契约就是它的职责，调用方猜不到的边界情况需要几句就写几句。它不该做的是复述实现——契约说完之后，还值得写的只剩「为什么是这个契约而不是别的」。函数体内没有契约要陈述，所以一条读者只要看下一行就能复原的注释比没有更糟——它在每一次阅读里都收一次时间，而那一行第一次在它没被同步的情况下改动时，文件就开始自相矛盾。这是本清单里唯一没有门禁的规则。词法检查无法把复述和解释分开，因为一条复述的注释用的是标识符里从来没有过的英文；它靠阅读来维持，这也正是它必须被写下来的原因。
14. **没有全局可变的服务状态。** 主题、宿主服务、时钟和资源都是显式的值。
15. **泛型消除重复的算法。** 它们不用来造组件类型层次或通用状态容器。当存储状态依赖某个类型时，把类型参数放在接收者类型上；当一个具体接收者拥有某项操作、但只有本次调用才引入类型时，把类型参数放在方法上。没有真实接收者的构造与适配仍然是包级函数；绝不为了使用方法语法而虚构工厂、工具或命名空间接收者。
16. **每个层级的一项语义操作只有一个入口。** 当可用零值或显式参数已经能让规范调用清晰时，不要再导出默认包装、`With` 变体、丢失信息的适配器或便利别名。第二个入口必须跨越一个说得清的边界——所有权、生命周期、抽象层级或结果契约——而不能只为了替调用方少写一个参数。契约变化时，更新调用方并删掉过时路线。
...
现有基础的大部分已经指向这个方向，都是要保住的资产：inline 发布、有界的 transcript 释放、带因果的输入流、有界字节摄入、故障注入的所有权结算、组件侧的原子呈现快照、事务化的滚动与 transcript 重排、活 widget 与被动 block 的区分、单拥有者运行时、复合的 dialog 与 tabs 控制器、结构化的组件语义、增量 markdown、裁剪过的 cell view、跨平台的尺寸变化观察、有界的宿主控制几何、消费方定义的能力、最小公开进程内程序宿主、带框的活单孩子组合、可执行的绘制纯度分类、发布 API 检查，以及被强制执行的依赖 DAG。
...
`Host.Input` 返回一个带因果的 `EventSource`：通道关闭之后跟着一个 `Err` 结果，于是干净的 EOF 与已知的传输失败保持可区分，而不用把 `program` 耦合到某个具体宿主。负责解码终端字节的 transport 会对每批 parser 输出调用一次 `input.Stamp`：协议解码拥有事件身份，transport 边界拥有到达时间。因此，按键与指针手势不可能在不同宿主中悄悄采用不同的时钟策略。`ByteIngress` 现在提供无损字节策略：它的子进程调用方证明了批量化、待处理字节上限、生产者背压、有序的完成与失败、以及拥有者取消，而没有改变通用分发。`Transcript.Commit` 已经在保住一个聚合的活坐标基准的同时，物理地释放已交付载荷与每块的放置记录；它的确定性保留测试和全新进程的 `N` 对 `2N` GC 测试，是这个切片必须保持绿色的关卡。故障注入覆盖来源与输入失败、一次歧义的部分输出写、失败之后不再写入、排空超时、能力缺席、有界关闭，以及尽力而为的真实终端拆除。

**这个切片已完成。** [`examples/streaming`](https://github.com/Tangerg/oolong/tree/main/examples/streaming) 是唯一的规范路径，而不是一个并行的陈列品：批准在来源启动之前转移焦点；来源只通过 `ByteIngress` 写；`markdown.Stream` 把变化中的后缀变成稳定块；带显式上限的 `Transcript.Commit` 只转移多出来的已完成前缀，同时保留一个固定大小的近期窗口用于选择、指针和滚动交互。取消是一个与来源失败不同的领域结果，拆除通过 ingress 所有权取消来源。确定性的示例测试证明批准顺序、稳定/开放的转换、保留上限、失败之前已接受的字节，以及取消。组件测试证明滚轮与选择的路由；真实 PTY 套件证明尺寸变化、空闲静默、模式恢复、scrollback 存活，以及从批准到发布的完整路径。program 与 terminal 的故障套件仍然是输入关闭、歧义部分写、排空超时、缺失可选能力和相互独立的拆除尝试的可执行证据。
...
**这个切片已完成。** `headless.Dialog` 拥有一个模态状态机；它的 content 和 trigger 是复合部件，每一条关闭路径都结算受控或本地的 open 状态，`Stack.Remove` 可以关掉一个被覆盖的对话框而不驱散更新的那一层。焦点测试证明打开会把键盘所有权从底层拿走，而每一种驱散都会把它还回去。层成员关系在焦点回调前提交；`Closed` 在移除之后、恢复焦点之前执行。同步回调可以重开或关闭对话框；新的焦点转移会终止旧转移尚未发出的通知。`headless.Tabs` 拥有自己的 tab 部件和受控或本地的选择；拥有者写入的受控状态通过一个显式的 `Sync` 应用，于是绘制永远不执行一次隐藏的焦点转移。两个控件都投影出同一棵带类型的 `SemanticNode` 树，而不暴露视觉盒子。`kit.NewDialog` 与 `kit.NewTabs` 是那条精致、可主题化的短路径，同时它们的控制器和外观部件仍然可达。结构化测试覆盖角色、selected/open/focused 状态和两种所有权模式。
...
- 相关构造设置是否使用了一个显式 `Config` 值，其中的可选字段具有实用的零值语义，而不是函数式选项？
- 这一层的每项语义操作是否只有一个入口？
- 如果两个入口共用实现，究竟是哪条所有权、生命周期、层级或结果边界让它们成为不同能力而不是别名？
