...
只要还有公开模块缺少最新版本的 tag，CI 就检查该版本的迁移清单。整组 tag 发布完成后，CI 恢复检查 `Unreleased`。
...
- 提议的 changelog 章节存在
- 每个不兼容的导出 API 变更，都以精确名称出现在所属模块对应发布版本的迁移清单中
...
API 比较由 `release.sh` 在每个 tag 发布前执行。tag 工作流使用
`gorelease -base=none -version=...` 验证模块可用性；发布后再让工具建议下一个版本，
检查结果就会依赖代理缓存时序。
...
保持原 tag 不变；在 `main` 上运行会检查不同的源码与依赖要求。
