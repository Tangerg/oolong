...
| mutable owners remain single owners | `internal/arch` derives every exported type that says it must not be copied from production documentation and requires a direct `noCopy`, `sync`, or `sync/atomic` field; `go vet` then rejects a value copy before shared mutable storage can acquire two owners | every CI run and every new no-copy contract |
