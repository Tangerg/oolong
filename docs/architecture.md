...
callback** runs from input handling. The library retains only functions it invokes;
an opaque caller value carries application behavior across a generic boundary without
making that function part of the rendering model. The library can enforce which phase
it calls and can reject intrinsic effects in its complete package-local render call
graph; it cannot inspect a downstream projection callback's body. Public callback
documentation and the caller's component tests therefore carry that final part of the
contract.
...
Time bounds must name which of two questions they answer. A **deadline** is an
ownership boundary: at the deadline instant the old operation is no longer live, so
the interval is half-open (`now < deadline`). An **interaction window** classifies an
event by when it arrived: an event no later than the stated limit may remain in the
gesture or key sequence, so the upper edge may be inclusive. The public documentation
and a boundary test must state that choice. When more than one operation decides the
same deadline ownership, they must share one predicate rather than repeat inverse
comparisons that can disagree at the exact instant.
...
`grid.View` owns clipped drawing intent, not an isolated slice of cell storage. An
operation must intersect its clip before it may act, but a multi-column display atom
is one indivisible storage and appearance fact. Replacing content through any of its
columns blanks the complete old atom, including the part outside the clip, while
preserving the outside columns' styles. Appearance belongs to the atom's head, the
only column emitted by the painter: an appearance area containing that head changes
the complete atom's style or link, including continuations outside the clip, while an
area containing only continuations does nothing. Adjacent areas therefore partition
appearance exactly once even for non-idempotent blends. These are the only permitted
mutations outside a view's clip, and the grid's white-box property tests enforce both
distinctions and the partition law.
...
| --- | --- | --- |
| dependency direction and vocabulary | `internal/arch` derives forbidden imports from the declared DAG and checks module promises, qualified and unqualified concrete-type references in documentation, completeness, and cycles | every CI run |
...
| explicit time-bound semantics | clipboard tests prove that request ownership is half-open and expires at its exact deadline; keymap tests prove that an interaction sequence includes a chord arriving exactly at its timeout. Both exercise the documented boundary rather than relying on scheduler timing | every new deadline or interaction window |
| observationally pure measurement and drawing | [`headless`](https://github.com/Tangerg/oolong/blob/main/components/headless/draw_purity_internals_test.go), [`kit`](https://github.com/Tangerg/oolong/blob/main/components/kit/draw_purity_internals_test.go), [`markdown`](https://github.com/Tangerg/oolong/blob/main/markdown/draw_purity_internals_test.go), and [`latex`](https://github.com/Tangerg/oolong/blob/main/latex/draw_purity_internals_test.go) derive every production `HeightForWidth` and `Draw*` receiver from source and execute each one twice; stateful cases preserve their semantic projection, all cases return the same extent and frame, and event callbacks are counted as part of state. The [`internal/arch` render-effects gate](https://github.com/Tangerg/oolong/blob/main/internal/arch/render_effects_internals_test.go) follows the type-resolved package-local call graph from those entries and rejects goroutines, I/O, clocks, randomness, logging, publication, and non-projection callbacks. The [callback-phase gate](https://github.com/Tangerg/oolong/blob/main/internal/arch/render_callbacks_internals_test.go) requires every retained function value to have exactly one non-stale projection, semantic, or event declaration | every package that implements measurement or drawing; an unclassified receiver, callback, intrinsic effect, unresolved local call, or phase violation fails |
...
| semantic state and focus have one owner | headless's source-derived controlled-state test requires every exported `Accessor` owner in that package to prove caller-written transitions, accepted writes and no-op writes. Kit's source-derived composition test requires every Config that accepts that ownership seam to have an executable two-way controlled example; [`internal/arch`](https://github.com/Tangerg/oolong/blob/main/internal/arch/facade_internals_test.go) separately rejects kit methods that mirror selected controller or editor state. Component ownership tests require repeated assignment of a demonstrably identical child and repeated focus reports to be idempotent, while the module-private identity primitive supplies the shared conservative comparison for open component interfaces | every CI run and every new controlled owner or component wrapper |
| one display-atom geometry | width fixtures are the single oracle for the deterministic estimator's ambiguous, emoji, combining, halfwidth-katakana, known spacing-mark boundary, and greater-than-two-column answers. One white-box fuzz target applies all six public cell-changing operations through full and clipped views; it proves that every head owns exactly its continuation columns, every column shares the atom's style and link, content replacement may only clear an intersected old atom outside the clip while preserving its styles, and appearance may only change a complete atom whose head the clip owns. A second differential target proves that applying Blend or Fade to adjacent partitions equals applying it once to their union. `ptytest.Screen` deliberately shares the estimator and independently models terminal-side application, pending wrap, overwrite, erasure, and scrolling | every text/grid test run and every width-table update |
| mutable owners remain single owners | `internal/arch` derives every exported type that says it must not be copied from production documentation and requires a direct `noCopy`, `sync`, or `sync/atomic` field. A second source-derived pass follows by-value struct containment across repository packages and rejects an exported wrapper that inherits copylocked identity without stating and marking its own ownership. `go vet` then rejects a value copy before shared mutable storage can acquire two owners | every CI run and every new mutable owner or no-copy contract |
...
| public module compatibility | every module builds without `go.work`; `scripts/modules.sh` derives complete membership from `go.work` and public membership from externally importable production packages across the supported source sets. The inventory is atomic and fail-closed: all source sets must be readable, an empty public set is invalid, diagnostics never enter the data stream, and consumers acquire a successful result before interpreting it. `internal/arch` exercises those failure and symlinked-checkout paths, independently reads Go's package facts, and requires both sets to be exact. The tested `internal/cmd/apiledger` Go command consumes that public set, reports rather than silently omits a module with no released baseline, uses pinned `apidiff` to compare each released module with its preceding immutable tag on Linux, macOS, and Windows, and requires every incompatible exported API change by exact API name in the selected migration ledger. The release workflow additionally runs pinned `gorelease`, reporting pre-1.0 changes and rejecting a proposed v1+ tag that violates Go compatibility | every CI run, the manual release check before tagging, and every public module tag |
| source portability and prose integrity | every module vets and builds for `wasip1/wasm` and `js/wasm`, proving its selected source and dependencies remain type-checkable on those targets without claiming either an I/O-free runtime or a WebAssembly terminal adapter; pinned CSpell checks the human-written Markdown and NOTICE vocabulary without diluting the gate with generated identifiers | every CI run and release |
