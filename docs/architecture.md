...
| public construction has one language | `internal/arch` rejects functional options, exported `Options` configuration, and undeclared multiple `New...` or `Open...` entries returning the same concrete type; a rare second resource-acquisition entry must name its exact ownership or lifecycle boundary, while ordinary ownership variants use one explicit `Config` value | every CI run |
