...
That is the whole reason this library exists rather than being a wrapper over an
existing one. Everything else here follows from it.

### What "in layers" buys

Three rings, each more general than the one above it:

| ring | knows about | must never touch |
| --- | --- | --- |
| `primitives/` | cells, graphemes, columns, escape sequences, the terminal, pacing, ranking | anything built from them |
| `atoms/` | widgets with no meaning of their own | goroutines, terminals, programs |
| `program/` | the loop, the frame schedule, the one goroutine | — |
...
| `overlay`, `theme` | `atoms/overlay`, `atoms/theme` | placement separated from drawing, so a hit test a frame later asks the same question |
| widgets living inside its runtime | `atoms/` | lifted out and made to mean nothing in particular |
...
is a scorer and a list, and that is what `primitives/fuzzy` and `atoms.Completion` are.
...
Widgets that mean nothing in particular: box and border, label, wrapped paragraph, row
and column layout with fixed/flex/measured sizing, spinner, scrollbar, scroll position,
key bindings and a help line, generic list, table, a multi-line editor with a kill ring
and coalesced undo, a floating layer with anchoring and clamping, mouse tracking that
commits a click on release over the target that took the press, a completion list, and a
semantic palette in `atoms/theme`.

Nothing in this ring imports the theme except the theme itself. Styles are explicit
fields on each widget, so a caller's palette is a caller's business.
...
- **A widget for everything.** The atoms are the ones a streaming interface actually
  needs. A library whose widget count is its selling point ends up with fifty widgets
  and no layering.
...
2. **A testing harness.** There is no equivalent of bubbletea's teatest or agentui's
   ptytest. The pty verification described above was a throwaway script, which is the
   wrong place for it: a library that wants people to test their interfaces should hand
   them the means. This is the largest gap for adopters.
3. **Examples.** None. A library with no `examples/` is one nobody can evaluate in five
   minutes, and five minutes is all anyone gives.
4. **A focus model.** Nothing assigns the keyboard to one of several widgets; a container
...
5. **Images and graphics.** Kitty and iTerm2 inline images, sixel. Real for an agent
   that reads screenshots, and last because it is the least general.
6. **Search over a transcript.** Product-shaped enough that it may never belong here.
