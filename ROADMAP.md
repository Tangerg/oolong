...
| **Somewhere to log while the terminal is taken** | `charmbracelet/log`, `tea.LogToFile` | Application policy, deliberately absent from `term`. The application opens its diagnostic sink with the standard library or its logging package and points its logger there; a terminal wrapper around `os.OpenFile` would create a second spelling without adding terminal semantics. |
